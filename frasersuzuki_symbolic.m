function varargout = frasersuzuki_symbolic(x,p)
   % y = frasersuzuki_symbolic(x,p)
   % [y, dydp, dydx] = frasersuzuki_symbolic(x,p)
   %
   ...
   %
   %
   % THIS FILE IS AN EXAMPLE HOW TO GENERATE INITIAL DISTRIBUTIONS FOR THE FPEX0 PACKAGE.
   %
   % Derivatives are computed using Matlab's symbolic computation toolbox.
   % If that toolbox is not available, the derivatives w.r.t. x and p must be coded manually.
   %
   %
