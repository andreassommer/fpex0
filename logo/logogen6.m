...
plot3(ax, x-0.7, zeros(size(x))-0.1, Z(1,:), 'Color', [1.0 0.0 0.4], 'LineWidth', 10);
...
view(ax, 303.0, 4.0);               % Blickwinkel: nach rechts hinten fluchtend
camlight('left');
lighting('gouraud');
material('shiny');
...
font3 = 'Cooper Black';
fontname = font3;
text(ax, -7, 13, max(Z(:))*1.03, 'FPEX_{0}', 'FontSize', 230, 'FontWeight', 'demi', ...
...
res = 200;
filename = sprintf('fpex0_logo_%d.png', res);
print(fig, filename,'-dpng',sprintf('-r%d', res));
