function dataStruct = DSCtools_readFile(fileSpec)
   % function data = DSCtools_readFile(fileSpec)
   %
   % Reads DSCtools CSV file and stores everything in data structure.
   %
   ...
   [desc, enc, fileHeaderStrings] = DSCtools_readFileHeader(fid, enc);
   ...
   columnHeaders = DSCtools_readColumnHeaders(fid, enc);
   
   % read data
   dataMat = DSCtools_readData(fid, enc, length(columnHeaders), NaN);
   ...
   dataStruct  = struct(...
                        'ID'           , IDENTITY              ,...
                        'rawData'      , rawData               ,...
                        'data'         , []                    ,...   % gets filled later by addQuickAccessors()
                        'rate'         , rate                  ,...
                        'mass'         , mass                  ,...
                        'desc'         , desc                  ,...
                        'Tinfo'        , Tinfo                  ...
   ...
   % add quick accessors
   dataStruct = DSCtools_addQuickAccessors(dataStruct, -inf, +inf);
   ...
   function Tinfo = extractTinfo(desc)
      try
         for k = 1:length(desc)
            Tinfo(k) = DSCtools_extractRANGE(desc(k).RANGE, enc);                 %#ok<AGROW>
         end
      catch
   ...
   function mass = extractSampleMass(desc)
      ...
      for k = 1:length(desc)
         try
            ...
            massString = DSCtools_replaceDecimalDelimiter(massString, enc);
