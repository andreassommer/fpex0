function Tinfo = DSCtools_extractRANGE(rangeString, enc)
   % Tinfo = DSCtools_extractRANGE(rangeString, enc)
   %
   % Extracts start and end temperatures as well as the stepping.
   % If Tstart > Tend, then Tstep will become negative.
   %
   % INPUT:   rangeString --> string from field "RANGE" of the DSCtools file header
   ...
   rangeString  = DSCtools_replaceDecimalDelimiter(rangeString, enc);
