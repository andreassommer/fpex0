function DSCsample = DSCtools_addCP(DSCsample, DSCreference, Tmin, Tmax, signalsource)
% DSCsample = DSCtools_addCP(DSCsample, DSCreference, Tmin, Tmax, signalsource)
%
...
scaleToRate = getSetting(DSCTOOLS_GLOBAL_SETTINGS, 'addCP_scaleToRate', true);  % scale signals to heatrate (higher rates induce higher signals, see below)
TrangeWarn  = getSetting(DSCTOOLS_GLOBAL_SETTINGS, 'addCP_TrangeWarn' , false); % warn if Tmin or Tmax exceed sample's or reference's temperature range

% process input args
if (nargin < 5 || isempty(signalsource)); signalsource = getSetting(DSCTOOLS_GLOBAL_SETTINGS, 'addCP_defaultSignalSource', 'uV'); end
if (nargin < 4 || isempty(Tmax))        ;         Tmax = getSetting(DSCTOOLS_GLOBAL_SETTINGS, 'addCP_defaultTmax',  inf); end % previously: 160
if (nargin < 3 || isempty(Tmin))        ;         Tmin = getSetting(DSCTOOLS_GLOBAL_SETTINGS, 'addCP_defaultTmin', -inf); end % previously:  55

   
   
% make function applicable for struct arrays
if length(DSCsample) > 1
   DSCsample = arrayfun(@(x) DSCtools_addCP(x, DSCreference, Tmin, Tmax, signalsource), DSCsample);
   return
end
...
% get the baseline
[blfun, bldata] = DSCtools_getBaseline(TS, cpS, 'linear');
cp.blfun  = blfun;
cp.bldata = bldata;

% build the latent cp function               DSCtools_subtractBaseline(X , Yin, blfun,clearzero,nonnegative, onset       , endset)
[latentCPvals, latentCPfun, latentCPfunpp] = DSCtools_subtractBaseline(TS, cpS, blfun, false   , true      , bldata.onset, bldata.endset);
%[latentCPvals,latentCPfun, latentCPfunpp] = DSCtools_subtractBaseline(TS, cpS, blfun, true    , true      , bldata.onset, bldata.endset);
