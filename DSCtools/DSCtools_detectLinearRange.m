function [stopidx,  reg, initreg] = DSCtools_detectLinearRange(X,Y,side,initlen,reldevAmax,reldevBmax,reldevS2max,absdevAmax,absdevBmax,absdevS2max)
% [stopidx, reg, initreg] = DSCtools_detectLinearRange(X,Y,side,initlen,reldevA,reldevB,reldevS2,absdevA,absdevB,absdevS2)
...
%             reg --> final regression structure as returned by DSCtools_linreg();
%         initreg --> initial regression structure as returned by DSCtools_linreg();
%
...
initreg = DSCtools_linreg(X(idx),Y(idx));
reg = initreg;

% walk through data: blockwise? maybe later.
for pos = runidx
   reg = DSCtools_linreg(X(pos),Y(pos),reg);  % update regression
