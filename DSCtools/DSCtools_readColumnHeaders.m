function columnHeaders = DSCtools_readColumnHeaders(fid, enc)
   % columnHeaders = DSCtools_readColumnHeaders(fid, enc)
   %
   % Searches and returns the column headers in an DSCtools export file.
   %
   ...
   while ~feof(fid)
      ...
      if (linelen>=2) && strcmp(line(1:2),'##')
         line = DSCtools_replaceChars(line, enc);  % replace strange characters in file header
