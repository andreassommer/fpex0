function [blfun, bldata] = DSCtools_getBaseline(X, Y, type, blds)
% [blfun, bldata] = DSCtools_getBaseline(X, Y, type, blds)
%
% Retrieves the baselevel function for specified DSCTOOLS_GLOBAL_SETTINGS.
%
...
if isempty(DSCtools_getBaseline_haltOnWarning)
   DSCtools_getBaseline_haltOnWarning = true;
end

% if no blds are specified, use defaults
if (nargin < 4),  blds = DSCtools_getBaselineDetectionSettings(); end
...
if (abs(reg_L.b) > maxslope) || (abs(reg_R.b) > maxslope)
   ...
   blfun = DSCtools_getBaselinePrimitive(X, Y, idx_L, reg_L.a, reg_L.b, idx_R, reg_R.a, reg_R.b, type);
   if DSCtools_getBaseline_haltOnWarning
