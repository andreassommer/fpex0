function DSCtoolstest_linreg()
   ...
   % full regression
   regYfull  = DSCtools_linreg(X,Ye);  % full regression
   regY2full = DSCtools_linreg(X,Y2e);
   ...
   idx = 1:fitfirst;
   regYp  = DSCtools_linreg(X(idx),Ye(idx));
   regY2p = DSCtools_linreg(X(idx),Y2e(idx));
   for k = (idx(end)+1):length(X)
      regYp  = DSCtools_linreg(X(k), Ye(k), regYp);
      regY2p = DSCtools_linreg(X(k), Y2e(k), regY2p);
